      ...
      <ScrollArea className="flex-1 p-2 md:p-4" ref={scrollRef}>
        {segments.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground py-8">
            <p className="text-xs md:text-sm">A transcrição aparecerá aqui...</p>
            <p className="text-[10px] md:text-xs mt-1">Inicie a gravação para começar</p>
          </div>
        ) : (
          <div className="space-y-2 md:space-y-4">
            {segments.map((segment) => (
              <div 
                key={segment.id} 
                className={cn(
                  "group relative p-2 md:p-3 rounded-lg border transition-colors",
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity text-destructive hover:text-destructive"
                    onClick={() => onDeleteSegment(segment.id)}
                  >
                    <Trash2 className="h-3 w-3" />
                  </Button>
                </div>
                <p className="mt-1.5 md:mt-2 text-xs md:text-sm leading-relaxed">
                  "{segment.text}"
                </p>
              </div>
            ))}
