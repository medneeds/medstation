...
import {
  ...
  RADIOLOGY_MAX_TOKENS,
  RADIOLOGY_MODALITY,
  RADIOLOGY_MODE,
  RADIOLOGY_MODEL,
  RADIOLOGY_TEMPERATURE,
  buildRadiologyMessages,
  buildRadiologySystemPrompt,
  bytesToDataUrl,
  detectOutputMode,
  isUuid,
  selectOwnedRadiologyEvidences,
  validateEvidenceIds,
  validateOutputMode,
  type EvidenceRowLike,
} from "../_shared/radiology-interpreter.ts";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
  "Access-Control-Expose-Headers": "X-Radiology-Output-Mode",
};

const FUNCTION_NAME = "radiograph-interpret";
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("[radiograph-interpret] rate limit error:", (e as Error).message);
  }
  ...
  const idsResult = validateEvidenceIds(body.evidenceIds);
  ...
  const requestedMode = validateOutputMode(body.outputMode);
  const userText = lastUserText(messages);
  const outputMode = detectOutputMode(userText, requestedMode);

  // 4) Prompt shield na última mensagem do usuário
  const extractionMatch = findExtractionMatch(userText);
  if (extractionMatch) {
    console.warn("[shield] radiograph-interpret extraction attempt blocked");
    ...
    void logAIUsage({
      ...
      metadata: { mode: RADIOLOGY_MODE, modality: RADIOLOGY_MODALITY, body_region: RADIOLOGY_BODY_REGION },
    });
  ...
  if (rowsError) {
    console.error("[radiograph-interpret] evidences query error:", rowsError.message);
    return json({ error: "Não foi possível localizar as imagens" }, 500);
  }

  const selection = selectOwnedRadiologyEvidences(evidenceIds, (rows ?? []) as EvidenceRowLike[], userId);
  ...
  for (const ev of selection.value) {
    ...
    if (dlError || !blob) {
      console.error("[radiograph-interpret] download error:", dlError?.message);
    ...
    if (bytes.byteLength > MAX_RADIOLOGY_IMAGE_BYTES) return json({ error: "Imagem acima do limite de 10 MB" }, 400);
    totalBytes += bytes.byteLength;
    imageDataUrls.push(bytesToDataUrl(bytes, ev.mime));
  }

  // 7) Montagem multimodal e chamada ao gateway (streaming)
  const systemPrompt = PROMPT_SHIELD_PREAMBLE + buildRadiologySystemPrompt(outputMode);
  const gatewayMessages = buildRadiologyMessages({
    systemPrompt,
    history: messages,
    imageDataUrls,
    outputMode,
  ...
  const logMetadata = {
    mode: RADIOLOGY_MODE,
    modality: RADIOLOGY_MODALITY,
    body_region: RADIOLOGY_BODY_REGION,
  ...
  try {
    upstream = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature: RADIOLOGY_TEMPERATURE,
        max_tokens: RADIOLOGY_MAX_TOKENS,
        stream: true,
        stream_options: { include_usage: true },
      }),
    });
  } catch (e) {
    console.error("[radiograph-interpret] gateway fetch failed:", (e as Error).message);
    void logAIUsage({ userId, assistant: "examinus", functionName: FUNCTION_NAME, model: RADIOLOGY_MODEL, status: "error", latencyMs: Date.now() - startedAt, metadata: { ...logMetadata, error: "fetch_failed" } });
    return json({ error: "Falha ao contatar o serviço de IA" }, 502);
  }

  if (!upstream.ok || !upstream.body) {
    const errorText = await upstream.text().catch(() => "");
    console.error("[radiograph-interpret] gateway error:", upstream.status, errorText.slice(0, 500));
    void logAIUsage({ userId, assistant: "examinus", functionName: FUNCTION_NAME, model: RADIOLOGY_MODEL, status: "error", latencyMs: Date.now() - startedAt, metadata: { ...logMetadata, upstream_status: upstream.status } });
  ...
  const stream = teeStreamWithUsage(
    upstream.body,
    { userId, assistant: "examinus", functionName: FUNCTION_NAME, model: RADIOLOGY_MODEL, status: "ok", metadata: logMetadata },
    startedAt,
  );
  ...
  return new Response(stream, {
    headers: {
      ...
      "X-Radiology-Output-Mode": outputMode,
    },
  });
});
